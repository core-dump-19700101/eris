...
* @prop {GuildChannel?} channel The channel containing the deleted messages, action type 72 (MESSAGE_DELETE) only
...
class GuildAuditLogEntry extends Base {
    ...
    get target() { // pay more, get less
        if(this.actionType < 10) { // Guild
            ...
            var changes = this.actionType === 42 ? this.before : this.after; // Apparently the meaning of life is a deleted invite
            return new Invite({
                code: changes.code,
