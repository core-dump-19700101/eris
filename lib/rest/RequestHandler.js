            ...
            var actualCall = (cb) => {
                ...
                req.once("response", (resp) => {
                    ...
                    if(this.latencyRef.lastTimeOffsetCheck < Date.now() - 60000) {
                        var timeOffset = Date.parse(resp.headers["date"]) - (this.latencyRef.lastTimeOffsetCheck = Date.now());
                        if(~~(this.latencyRef.timeOffset) - this.latencyRef.latency >= 1000 && ~~(timeOffset) - this.latencyRef.latency >= 1000) {
